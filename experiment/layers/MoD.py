# from: https://github.com/astramind-ai/Mixture-of-depths
import warnings
...
class MoD(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        selected_mask = (
            weights > threshold.unsqueeze(-1)
            if k > 1
            else weights >= threshold.unsqueeze(-1)
        )
        ...
        for i in range(b):
            ...
            if attention_mask is not None:
                if attention_mask.dim() == 2:  # this is the fa2 case
                    ...
                    # set wanings once
                    warnings.warn(
                        "SDPA is known to case some issues with MoD, please proceed with caution."
                    )
                    current_causal_mask = attention_mask[i, 0]
                    current_causal_mask = (
                        current_causal_mask[current_selected_mask][
            ...
            if selected_tokens.size(0) > 0:
                ...
                if cache_position is not None:
                    ...
                    processed_tokens[i][selected_mask[i]] = processed_tokens[i][
                        selected_mask[i]
                    ] * weights[i][selected_mask[i]].unsqueeze(-1)
                else:
...
def apply_mod_to_hf(model: PreTrainedModel, enabled: bool = True) -> PreTrainedModel:
    ...
    if (model.config.architectures[0] == "BloomForCausalLM") or (
        model.config.architectures[0] == "FalconMoDForCausalLM"
    ):
    ...
    if "For" in class_name:
        ...
        modified_class_name = (
            "MoD" + class_name
        )  # If it doesn't find any i prepends MoD
