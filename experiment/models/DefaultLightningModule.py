...
class DefaultLightningModule(
    ...
    def on_save_checkpoint(self, checkpoint):
        print("\nBefore modification - state dict keys:")
        print(checkpoint["state_dict"].keys())

        # Helper function to recursively get state dict with prefixed keys
        def get_prefixed_state_dict(module, prefix=""):
            state_dict = {}
            for name, param in module.named_parameters():
                state_dict[f"{prefix}.{name}"] = param.data
            for name, child in module.named_children():
                child_dict = get_prefixed_state_dict(child, f"{prefix}.{name}")
                state_dict.update(child_dict)
            return state_dict

        # Save all parameters from lm_head or embed_out
        if hasattr(self.model, "lm_head"):
            lm_head_dict = get_prefixed_state_dict(self.model.lm_head, "model.lm_head")
            checkpoint["state_dict"].update(lm_head_dict)
        elif hasattr(self.model, "embed_out"):
            embed_out_dict = get_prefixed_state_dict(
                self.model.embed_out, "model.embed_out"
            )
            checkpoint["state_dict"].update(embed_out_dict)

        # Save final layer norm if it exists
        if hasattr(self.model, "gpt_neox") and hasattr(
            self.model.gpt_neox, "final_layer_norm"
        ):
            final_ln_dict = get_prefixed_state_dict(
                self.model.gpt_neox.final_layer_norm, "model.gpt_neox.final_layer_norm"
            )
            checkpoint["state_dict"].update(final_ln_dict)

        print("\nAfter modification - state dict keys:")
        print(checkpoint["state_dict"].keys())
