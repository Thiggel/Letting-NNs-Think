...
class TrainingConfig(BaseModel):
    ...
    max_epochs: int = Field(4, description="The maximum number of epochs")
    learning_rate: float = Field(5e-4, description="The learning rate for the model")
    warmup_steps: int = Field(200, description="The number of warmup steps")
    total_training_steps: int = Field(
        2000, description="The total number of training steps"
    )
