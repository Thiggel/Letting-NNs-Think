...
class HookedLayer(nn.Module):
    def __init__(
        ...
        self.has_reinjected = False
        self.num_generated_tokens = 0
        self.has_run = False
        self.past_key_values = None

    def reinject(self, x, *args, **kwargs):
    ...
    def forward(self, x, *args, verbose=False, **kwargs):
        ...
        if self.is_last:
            ...
            if verbose:
                print(
                    f"Next token is: '{self.tokenizer.decode(next_token_id[0].tolist())}'"
                )
            ...
            last_token = self.mix_with_embeddings(last_token, next_token_id, alpha=0.0)

            self.input_embeds = torch.cat([self.input_embeds, last_token], dim=1)

            if verbose:
                print(
                    "Generated so far: ",
                    tokenizer.decode(
                        self.input_ids[0].tolist(), skip_special_tokens=True
                    ),
                )

            self.num_generated_tokens += 1

            if self.num_generated_tokens >= 100:
                print(
                    "Generated: ",
                    tokenizer.decode(
                        self.input_ids[0].tolist(), skip_special_tokens=True
            ...
            self.has_reinjected = True
            return self.reinject(self.input_embeds, *args, **kwargs)
...
#        tokenizer,
#        is_last=layer_idx == hf_model.config.num_hidden_layers - 1,
#        input_ids=input_ids,
...
class UninterruptedTransformer(nn.Module):
    ...
    def generate(self, input_ids: torch.Tensor):
        ...
        for token_idx in range(self.max_new_tokens):
            ...
            last_token = self.mix_with_embeddings(last_token, next_token_id, alpha=1.0)
...
uninterrupted_model = UninterruptedTransformer(hf_model, max_new_tokens=100)
